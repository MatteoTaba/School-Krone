(function() {
    ...
    1: [
        function(require, module1, exports) {
            (function(global) {
                ...
                if (typeof Web3 == "object" && Object.keys(Web3).length == 0) Web3 = global.Web3;
                var contract1 = function(module) {
                    ...
                    var Utils = {
                        ...
                        decodeLogs: function(C, instance, logs) {
                            return logs.map(function(log) {
                                ...
                                copy.args = logABI.inputs.reduce(function(acc, current) {
                                    ...
                                    if (val === undefined) val = notIndexedParams[current.name];
                                ...
                                Object.keys(copy.args).forEach(function(key) {
                                    ...
                                    if (val.constructor.isBN) copy.args[key] = C.web3.toBigNumber("0x" + val.toString(16));
                                });
                                delete copy.data;
                                delete copy.topics;
                                return copy;
                        ...
                        promisifyFunction: function(fn, C) {
                            ...
                            return function() {
                                ...
                                if (Utils.is_object(last_arg) && !Utils.is_big_number(last_arg)) tx_params = args.pop();
                                ...
                                return C.detectNetwork().then(function() {
                                    return new Promise(function(accept, reject) {
                                        var callback = function(error, tx) {
                                            ...
                                            var timeout;
                                            if (C.synchronization_timeout === 0 || C.synchronization_timeout !== undefined) timeout = C.synchronization_timeout;
                                            else timeout = 240000;
                                            var start = (new Date).getTime();
                                            var make_attempt = function() {
                                                C.web3.eth.getTransactionReceipt(tx, function(err, receipt) {
                                                    if (err && !err.toString().includes("unknown transaction")) return reject(err);
                                                    if (receipt != null) {
                        ...
                        parallel: function(arr, callback) {
                            ...
                            if (!arr.length) return callback(null, []);
                            var index = 0;
                            ...
                            arr.forEach(function(fn, position) {
                                fn(function(err, result) {
                                    if (err) {
                                        ...
                                        if (index >= arr.length) callback(null, results);
                                    }
                                });
                            });
                        },
                    ...
                    function Contract(contract) {
                        ...
                        for(var i = 0; i < this.abi.length; i++){
                            ...
                            if (item.type == "function") {
                                if (item.constant == true) this[item.name] = Utils.promisifyFunction(contract[item.name], constructor);
                                else this[item.name] = Utils.synchronizeFunction(contract[item.name], this, constructor);
                    ...
                    Contract._static_methods = {
                        setProvider: function(provider) {
                            if (!provider) throw new Error("Invalid provider passed to setProvider(); provider is " + provider);
                        ...
                        new: function() {
                            var self = this;
                            if (this.currentProvider == null) throw new Error(this.contractName + " error: Please call setProvider() first before calling new().");
                            var args = Array.prototype.slice.call(arguments);
                            if (!this.bytecode) throw new Error(this._json.contractName + " error: contract binary not set. Can't deploy new instance.");
                            return self.detectNetwork().then(function(network_id) {
                                ...
                                return new Promise(function(accept, reject) {
                                    ...
                                    if (constructor.length && constructor[0].inputs.length !== args.length) throw new Error(self.contractName + " contract constructor expected " + constructor[0].inputs.length + " arguments, received " + args.length);
                                    tx_params = Utils.merge(self.class_defaults, tx_params);
                                    if (tx_params.data == null) tx_params.data = self.binary;
                                    var intermediary = function(err, web3_instance) {
                                        if (err != null) {
                                            reject(err);
                                            return;
                                        }
                                        if (err == null && web3_instance != null && web3_instance.address != null) accept(new self(web3_instance));
                                    };
                        ...
                        at: function(address) {
                            var self = this;
                            if (address == null || typeof address != "string" || address.length != 42) throw new Error("Invalid address passed to " + this._json.contractName + ".at(): " + address);
                                ...
                                return self.detectNetwork().then(function(network_id) {
                                    ...
                                    return new Promise(function(accept, reject) {
                                        self.web3.eth.getCode(address, function(err, code) {
                                            ...
                                            if (!code || code.replace("0x", "").replace(/0/g, "") === "") return reject(new Error("Cannot create instance of " + self.contractName + "; no code at address " + address));
                                            accept(instance);
                                        });
                                    });
                                }).then(fn);
                        ...
                        deployed: function() {
                            ...
                            return self.detectNetwork().then(function() {
                                if (self._json.networks[self.network_id] == null) throw new Error(self.contractName + " has not been deployed to detected network (network/artifact mismatch)");
                                if (!self.isDeployed()) throw new Error(self.contractName + " has not been deployed to detected network (" + self.network_id + ")");
                                return new self(self.address);
                            });
                        },
                        defaults: function(class_defaults) {
                            if (this.class_defaults == null) this.class_defaults = {
                            };
                        ...
                        isDeployed: function() {
                            if (this.network_id == null) return false;
                        ...
                        detectNetwork: function() {
                            ...
                            return new Promise(function(accept, reject) {
                                if (self.network_id) {
                                    if (self.networks[self.network_id] != null) return accept(self.network_id);
                                }
                                self.web3.version.getNetwork(function(err1, result) {
                        ...
                        link: function(name1, address) {
                            var self = this;
                            if (typeof name1 == "function") {
                                var contract = name1;
                                if (contract.isDeployed() == false) throw new Error("Cannot link contract without an address.");
                            ...
                            if (this._json.networks[this.network_id] == null) this._json.networks[this.network_id] = {
                                events: {
                                },
                                links: {
                                }
                            };
                            this.network.links[name1] = address;
                        },
                        clone: function(json) {
                            var self = this;
                            json = json || {
                            ...
                            if (network_id) temp.setNetwork(network_id);
                        ...
                        addProp: function(key, fn) {
                            var self = this;
                            var getter = function() {
                                if (fn.get != null) return fn.get.call(self);
                    ...
                    Contract._properties = {
                        ...
                        network: function() {
                            ...
                            if (network_id == null) throw new Error(this.contractName + " has no network id set, cannot lookup artifact data. Either set the network manually using " + this.contractName + ".setNetwork(), run " + this.contractName + ".detectNetwork(), or use new(), at() or deployed() as a thenable which will detect the network automatically.");
                            if (this._json.networks[network_id] == null) throw new Error(this.contractName + " has no network configuration for its current network id (" + network_id + ").");
                            var returnVal = this._json.networks[network_id];
                            if (returnVal.links == null) returnVal.links = {
                            };
                            if (returnVal.events == null) returnVal.events = {
                            };
                            return returnVal;
                        },
                        networks: function() {
                        ...
                        address: {
                            get: function() {
                                ...
                                if (address == null) throw new Error("Cannot find deployed address: " + this.contractName + " not deployed or address not set.");
                                return address;
                            },
                            set: function(val) {
                                if (val == null) throw new Error("Cannot set deployed address; malformed value: " + val);
                                var network_id = this.network_id;
                                if (network_id == null) throw new Error(this.contractName + " has no network id set, cannot lookup artifact data. Either set the network manually using " + this.contractName + ".setNetwork(), run " + this.contractName + ".detectNetwork(), or use new(), at() or deployed() as a thenable which will detect the network automatically.");
                                if (this._json.networks[network_id] == null) this._json.networks[network_id] = {
                                    events: {
                                    },
                                    links: {
                                    }
                        ...
                        transactionHash: {
                            get: function() {
                                ...
                                if (transactionHash === null) throw new Error("Could not find transaction hash for " + this.contractName);
                                return transactionHash;
                            },
                            set: function(val) {
                                if (val === null) throw new Error("Could not set `" + val + "` as the transaction hash for " + this.contractName);
                                this.network.transactionHash = val;
                            }
                        },
                        links: function() {
                            if (!this.network_id) throw new Error(this.contractName + " has no network id set, cannot lookup artifact data. Either set the network manually using " + this.contractName + ".setNetwork(), run " + this.contractName + ".detectNetwork(), or use new(), at() or deployed() as a thenable which will detect the network automatically.");
                            if (this._json.networks[this.network_id] == null) return {
                            };
                        ...
                        events: function() {
                            var web3 = new Web3;
                            var events;
                            if (this._json.networks[this.network_id] == null) events = {
                            };
                            else events = this.network.events || {
                            };
                            var abi = this.abi;
                            abi.forEach(function(item) {
                                ...
                                item.inputs.forEach(function(input, index) {
                                    ...
                                    if (index < item.inputs.length - 1) signature += ",";
                                });
                                signature += ")";
                        ...
                        deployedBytecode: {
                            get: function() {
                                ...
                                if (code.indexOf("0x") != 0) code = "0x" + code;
                                return code;
                            },
                            set: function(val) {
                                var code = val;
                                if (val.indexOf("0x") != 0) code = "0x" + code;
    ...
    10: [
        function(require, module, exports) {
            ...
            module.exports = compile1;
            function compile1(schema1, root, localRefs1, baseId1) {
                var self = this, opts = this._opts, refVal1 = [
                    undefined
                ], refs = {
                }, patterns = [], patternsHash = {
                ...
                var c = checkCompiling.call(this, schema1, root, baseId1);
                var compilation = this._compilations[c.index];
                if (c.compiling) return compilation.callValidate = callValidate;
                var formats = this._formats;
                var RULES = this.RULES;
                try {
                    var v = localCompile(schema1, root, localRefs1, baseId1);
                    ...
                    endCompiling.call(this, schema1, root, baseId1);
                }
                function callValidate() {
                ...
                function localCompile(_schema, _root, localRefs, baseId) {
                    ...
                    if (_root.schema != root.schema) return compile1.call(self, _schema, _root, localRefs, baseId);
                    ...
                    sourceCode = vars(refVal1, refValCode) + vars(patterns, patternCode) + vars(defaults, defaultCode) + vars(customRules, customRuleCode) + sourceCode;
                    ...
                    try {
                        ...
                        validate = makeValidate(self, RULES, formats, root, refVal1, defaults, customRules, co, equal, ucs2length, ValidationError);
                        refVal1[0] = validate;
                    } catch (e) {
                ...
                function resolveRef(baseId, ref, isRoot) {
                    ...
                    if (v === undefined) {
                        var localSchema = localRefs1 && localRefs1[ref];
                        if (localSchema) v = resolve.inlineRef(localSchema, opts.inlineRefs) ? localSchema : compile1.call(self, localSchema, root, localRefs1, baseId);
                    }
        ...
        function(require, module, exports) {
            ...
            module.exports = function generate_format(it, $keyword, $ruleType) {
                ...
                if ($isData) {
                    ...
                    if (it.async) out += " (async" + $lvl + " ? " + it.yieldAwait + " " + $format + "(" + $data + ") : " + $format + "(" + $data + ")) ";
    ...
    33: [
        function(require, module, exports) {
            ...
            module.exports = function generate_properties(it, $keyword, $ruleType) {
                ...
                }, $pPropertyKeys = Object.keys($pProperties), $aProperties = it.schema.additionalProperties, $someProperties = $schemaKeys.length || $pPropertyKeys.length, $noAdditional = $aProperties === false, $additionalIsSchema = typeof $aProperties == "object" && Object.keys($aProperties).length, $removeAdditional = it.opts.removeAdditional, $checkAdditional = $noAdditional || $additionalIsSchema || $removeAdditional, $ownProperties = it.opts.ownProperties, $currentBaseId = it.baseId;
                var $required = it.schema.required;
                if ($required && !(it.opts.v5 && $required.$data) && $required.length < it.opts.loopRequired) var $requiredHash = it.util.toHash($required);
                if (it.opts.patternGroups) var $pgProperties = it.schema.patternGroups || {
                                ...
                                if ($pgMin !== undefined || $pgMax !== undefined) {
                                    ...
                                    if ($pgMin !== undefined) {
                                        ...
                                        if (it.createErrors !== false) {
                                            ...
                                            if (it.opts.messages !== false) out += " , message: 'should NOT have " + $moreOrLess + " than " + $limit + ' properties matching pattern "' + it.util.escapeQuotes($pgProperty) + "\"' ";
                                            if (it.opts.verbose) out += " , schema: validate.schema" + $schemaPath + " , parentSchema: validate.schema" + it.schemaPath + " , data: " + $data + " ";
                                            out += " } ";
                                        } else out += " {} ";
                                        var __err = out;
                    ...
                    if ($schema.length < it.opts.loopRequired && it.schema.properties && Object.keys(it.schema.properties).length) {
                        ...
                        if (arr1) {
                            ...
                            while(i1 < l1){
                                ...
                                if (!($propertySch && it.util.schemaHasRules($propertySch, it.RULES.all))) $required[$required.length] = $property;
                            }
                        }
        ...
        function(require, module, exports) {
            ...
            module.exports = function generate_validate(it, $keyword, $ruleType) {
                        ...
                        if ($coerceToTypes) {
                            ...
                            if (arr1) {
                                ...
                                while($i < l1){
                                    ...
                                    if ($type == "string") out += " if (" + $dataType + " == 'number' || " + $dataType + " == 'boolean') " + $coerced + " = '' + " + $data + "; else if (" + $data + " === null) " + $coerced + " = ''; ";
                                    else if ($type == "number" || $type == "integer") {
                                        out += " if (" + $dataType + " == 'boolean' || " + $data + " === null || (" + $dataType + " == 'string' && " + $data + " && " + $data + " == +" + $data + " ";
                                        if ($type == "integer") out += " && !(" + $data + " % 1)";
                                        out += ")) " + $coerced + " = +" + $data + "; ";
                                    } else if ($type == "boolean") out += " if (" + $data + " === 'false' || " + $data + " === 0 || " + $data + " === null) " + $coerced + " = false; else if (" + $data + " === 'true' || " + $data + " === 1) " + $coerced + " = true; ";
                                    else if ($type == "null") out += " if (" + $data + " === '' || " + $data + " === 0 || " + $data + " === false) " + $coerced + " = null; ";
                                    else if (it.opts.coerceTypes == "array" && $type == "array") out += " if (" + $dataType + " == 'string' || " + $dataType + " == 'number' || " + $dataType + " == 'boolean' || " + $data + " == null) " + $coerced + " = [" + $data + "]; ";
                                }
                            }
                ...
                if (it.schema.$ref && !$refKeywords) {
                    ...
                    if (it.opts.v5 && it.schema.patternGroups) it.logger.warn('keyword "patternGroups" is deprecated and disabled. Use option patternGroups: true to enable.');
                    var arr2 = it.RULES;
                    if (arr2) {
    ...
    39: [
        function(require, module, exports) {
            ...
            function addKeyword(keyword1, definition1) {
                var RULES = this.RULES;
                if (RULES.keywords[keyword1]) throw new Error("Keyword " + keyword1 + " is already defined");
                if (!IDENTIFIER.test(keyword1)) throw new Error("Keyword " + keyword1 + " is not a valid identifier");
                if (definition1) {
                    if (definition1.macro && definition1.valid !== undefined) throw new Error('"valid" option cannot be used with macro keywords');
                    var dataType = definition1.type;
                    if (Array.isArray(dataType)) {
                        var i, len = dataType.length;
                        for(i = 0; i < len; i++)checkDataType(dataType[i]);
                        for(i = 0; i < len; i++)_addRule(keyword1, dataType[i], definition1);
                    } else {
                        if (dataType) checkDataType(dataType);
                        _addRule(keyword1, dataType, definition1);
                    }
                    var $data = definition1.$data === true && this._opts.$data;
                    if ($data && !definition1.validate) throw new Error('$data support: "validate" function is not defined');
                    var metaSchema = definition1.metaSchema;
                    if (metaSchema) {
                        if ($data) metaSchema = {
                            anyOf: [
                                metaSchema,
                        ...
                        definition1.validateSchema = this.compile(metaSchema, true);
                    }
                }
                RULES.keywords[keyword1] = RULES.all[keyword1] = true;
    ...
    44: [
        function(require, module2, exports1) {
        ...
        function(require, module, exports) {
            (function(Buffer) {
                ...
                function encodeParams(types, values) {
                    if (types.length !== values.length) throw new Error("[ethjs-abi] while encoding params, types/values mismatch, types length " + types.length + " should be " + values.length);
                    var parts = [];
                    ...
                    parts.forEach(function(part) {
                        ...
                        } else staticSize += alignSize(part.value.length);
                    });
        ...
        function(require, module, exports) {
            (function(Buffer) {
                ...
                function hexlify(value) {
                    if (typeof value === "number") return "0x" + bnToBuffer(new BN(value)).toString("hex");
                    else if (value.mod || value.modulo) return "0x" + bnToBuffer(value).toString("hex");
                ...
                function getKeys(params, key, allowEmpty) {
                    ...
                    for(var i = 0; i < params.length; i++){
                        ...
                        else if (typeof value !== "string") throw new Error("[ethjs-abi] while getKeys found invalid ABI data structure, type value not string");
                        result.push(value);
                    }
                    return result;
                }
                function coderNumber(size, signed) {
                    return {
                        encode: function encodeNumber(valueInput) {
                            var value = valueInput;
                            if (typeof value === "object" && value.toString && (value.toTwos || value.dividedToIntegerBy)) value = value.toString(10).split(".")[0];
                            if (typeof value === "string" || typeof value === "number") value = String(value).split(".")[0];
                            value = numberToBN(value);
                            value = value.toTwos(size * 8).maskn(size * 8);
                            if (signed) value = value.fromTwos(size * 8).toTwos(256);
                        ...
                        decode: function decodeNumber(data, offset) {
                            ...
                            if (signed) value = value.fromTwos(size * 8);
                            else value = value.maskn(size * 8);
                            return {
                                consumed: 32,
                    ...
                    while(type){
                        ...
                        switch(prefix){
                            ...
                            case "uint":
                                ...
                                if (intSize === 0 || intSize > 256 || intSize % 8 !== 0) throw new Error("[ethjs-abi] while getting param coder for type " + type + ", invalid " + prefix + "<N> width: " + type);
                            ...
                            case "bytes":
                                ...
                                if (part[3]) {
                                    ...
                                    if (size === 0 || size > 32) throw new Error("[ethjs-abi] while getting param coder for prefix bytes, invalid type " + type + ", size " + size + " should be 0 or greater than 32");
                                    coder = coderFixedBytes(size);
                                } else coder = coderDynamicBytes;
                                break;
                            case "address":
                                if (coder) throw new Error(invalidTypeErrorMessage);
                                coder = coderAddress;
            ...
            (function(process, global) {
                (function(root) {
                    ...
                    for(var i3 = 0; i3 < algorithms.length; ++i3){
                        ...
                        for(var j = 0; j < bits1.length; ++j)methods[algorithm.name + "_" + bits1[j]] = algorithm.createMethod(bits1[j], algorithm.padding);
                    }
                    ...
                    Keccak.prototype.toString = Keccak.prototype.hex = function() {
                        ...
                        if (extraBytes) {
                            block = s[i];
                            if (extraBytes > 0) hex += HEX_CHARS[block >> 4 & 15] + HEX_CHARS[block & 15];
                    ...
                    else if (root) for(var key in methods)root[key] = methods[key];
                })(this);
                ...
                if (schema && typeof schema == "object" && !Array.isArray(schema)) {
                    ...
                    for(var key in schema){
                        ...
                        if (Array.isArray(sch)) {
                            if (key in traverse.arrayKeywords) for(var i = 0; i < sch.length; i++)_traverse(opts, cb, sch[i], jsonPtr + "/" + key + "/" + i, rootSchema, jsonPtr, key, schema, i);
                        } else if (key in traverse.propsKeywords) {
                            if (sch && typeof sch == "object") for(var prop in sch)_traverse(opts, cb, sch[prop], jsonPtr + "/" + key + "/" + escapeJsonPtr(prop), rootSchema, jsonPtr, key, schema, prop);
                        } else if (key in traverse.keywords || opts.allKeys && !(key in traverse.skipKeywords)) _traverse(opts, cb, sch, jsonPtr + "/" + key, rootSchema, jsonPtr, key, schema);
                    }
                }
            }
        ...
        function(require, module, exports) {
            var Blockchain = {
                ...
                asURI: function(provider, callback) {
                    var self = this;
                    var genesis;
                    self.getBlockByNumber("0x0", provider, function(err2, response1) {
                        if (err2) return callback(err2);
                        genesis = response1.result;
                ...
                matches: function(uri, provider, callback) {
                    ...
                    self.getBlockByNumber("0x0", provider, function(err3, response2) {
                        if (err3) return callback(err3);
                        var block1 = response2.result;
                        if (block1.hash != expected_genesis) return callback(null, false);
                        self.getBlockByHash(expected_block, provider, function(err, response) {
                            var block = response.result;
                            if (err || block == null) return callback(null, false);
                            callback(null, true);
                        });
                    });
                }
        ...
        function(require, module, exports) {
            ...
            var properties = {
                ...
                bytecode: {
                    ...
                    transform: function(value) {
                        if (value && value.indexOf("0x") != 0) value = "0x" + value;
                        return value;
                    }
                },
                deployedBytecode: {
                ...
                legacyAST: {
                    transform: function(value, obj) {
                        ...
                        if (schemaVersion[0] < 2) return obj.ast;
                        else return value;
                    }
                },
                compiler: {
            ...
            function getter(key, transform) {
                if (transform === undefined) transform = function(x) {
                    return x;
                };
                return function(obj) {
                    try {
            ...
            var TruffleContractSchema = {
                validate: function(contractObj) {
                    ...
                    if (ajv.validate("contract-object.spec.json", contractObj)) return contractObj;
                    else throw ajv.errors;
                },
                normalize: function(objDirty, options) {
                    ...
                    Object.keys(properties).forEach(function(key) {
                        ...
                        if (property.transform) value = property.transform(value, objDirty);
                        normalized[key] = value;
                    });
                    Object.keys(objDirty).forEach(function(key) {
                        if (key.indexOf("x-") === 0) normalized[key] = getter(key)(objDirty);
                    });
                    normalized.schemaVersion = pkgVersion;
                    if (options.validate) this.validate(normalized);
                    return normalized;
                }
            };
    ...
    66: [
        function(require, module, exports) {
            (function(root, factory, undef) {
                if (typeof exports === "object") module.exports = exports = factory(require("./core"), require("./x64-core"));
    ...
    67: [
        function(require, module, exports) {
            (function(root, factory) {
                if (typeof exports === "object") module.exports = exports = factory(require("./core"));
                else if (typeof define === "function" && define.amd) define([
                    "./core"
                ], factory);
                else factory(root.CryptoJS);
    ...
    72: [
        function(require, module, exports) {
            function ExtendableBuiltin1(cls) {
            ...
            module.exports = ExtendableBuiltin1;
        },
        {
        }
    ],
    ...
    79: [
        function(require, module, exports) {
            ...
            function StatusError(args, tx, receipt) {
                ...
                if (receipt.gasUsed === gasLimit) message = "Transaction: " + tx + " exited with an error (status 0) after consuming all gas.\n" + "Please check that the transaction:\n" + "    - satisfies all conditions set by Solidity `assert` statements.\n" + "    - has enough gas to execute the full transaction.\n" + "    - does not trigger an invalid opcode by other means (ex: accessing an array out of bounds).";
                else message = "Transaction: " + tx + " exited with an error (status 0).\n" + "Please check that the transaction:\n" + "    - satisfies all conditions set by Solidity `require` statements.\n" + "    - does not trigger a Solidity `revert` statement.\n";
...
//# sourceMappingURL=upload-page.036c93e6.js.map
